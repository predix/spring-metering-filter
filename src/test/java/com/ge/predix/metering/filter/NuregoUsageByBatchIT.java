...
public class NuregoUsageByBatchIT extends AbstractTestNGSpringContextTests {
    ...
    Map<String, Double> beforeUsage = new HashMap<>();

    @BeforeClass
    public void setup() {
        try {
            createSubscription(ACS_PLAN_ID, ACS_SUBSCRIPTION_ID);
            beforeUsage.put("policy_eval" + ACS_SUBSCRIPTION_ID, 
                    getEntitlementUsageByFeatureId("policy_eval", ACS_SUBSCRIPTION_ID));
            beforeUsage.put("policyset_update" + ACS_SUBSCRIPTION_ID, 
                    getEntitlementUsageByFeatureId("policyset_update", ACS_SUBSCRIPTION_ID));
            
            createSubscription(UAA_PLAN_ID, UAA_SUBSCRIPTION_ID);
            beforeUsage.put("number_of_tokens" + UAA_SUBSCRIPTION_ID, 
                    getEntitlementUsageByFeatureId("number_of_tokens", UAA_SUBSCRIPTION_ID));
            beforeUsage.put("number_of_users" + UAA_SUBSCRIPTION_ID, 
                    getEntitlementUsageByFeatureId("number_of_users", UAA_SUBSCRIPTION_ID));
        } catch (Exception e) {
            e.printStackTrace();
            Assert.fail();
        }
    ...
    @Test(dependsOnMethods="testNuregoIntegration", dataProvider = "requestProvider")
    public void testAssertReportedUsage(final String featureId, final String planId, final String subscriptionId,
            final ServletRequest request, final ServletResponse response) throws Exception {
        Thread.sleep(2000);
        Double beforeUsedAmount = beforeUsage.get(featureId + subscriptionId);
        Double afterUsedAmount = getEntitlementUsageByFeatureId(featureId, subscriptionId);
        Assert.assertEquals(afterUsedAmount - beforeUsedAmount, 1.0);
    }

    private void createSubscription(final String planId, final String subscriptionId)
            throws AuthenticationException, APIConnectionException, APIException, InvalidRequestException {
